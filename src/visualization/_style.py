...
def get_plotly_font_family() -> str:
    """Return a cross-platform Chinese font family string for Plotly charts.

    Unlike matplotlib (which uses rcParams), Plotly accepts a CSS font-family
    string. This function detects the OS and returns the best available font
    stack, falling back to generic sans-serif.
    """
    system = platform.system()
    if system == "Windows":
        candidates = ["Microsoft YaHei", "SimHei", "SimSun"]
    ...
    if chosen:
        return f"'{chosen}', sans-serif"
    return ", ".join(f"'{c}'" for c in candidates) + ", sans-serif"
